...
export default function RioClient({ data }) {
  const [filter, setFilter] = useState("all");

  const alerts = useMemo(
    () => [...(data.alerts || [])].sort((a, b) => b.date.localeCompare(a.date)),
    [data]
  );
  ...
  const shown =
    filter === "all" ? alerts : alerts.filter((a) => a.personId === filter);
    ...
    <div>
      <div className="page-head">
        ...
        <p>
          ...
          Último barrido: {formatDate(data.lastSweep, false)} · {alerts.length}{" "}
          señales.
        </p>
      </div>
